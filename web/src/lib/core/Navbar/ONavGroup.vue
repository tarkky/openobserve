...
const activeChild = computed<SubnavChild | null>(() => {
  ...
  // `!c.tab` is main's narrowing, kept deliberately: a tab child that did not
  // match `exactTab` above is handled by the `defaultForRoute` fallback below,
  // so admitting it here would claim the wrong child before that runs. Our
  // `routeAlias` is orthogonal to that — a different mechanism for sibling
  // routes rather than query tabs — so both belong.
